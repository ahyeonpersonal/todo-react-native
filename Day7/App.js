...
// uuid : Universally Unique Identifier 128 bit number that identifies unique internet object or data
...
export default class App extends React.Component {
  ...
  _addToDo =()=>{
    ...
    if(newToDo !== ""){
      ...
      Object =>  since 'newToDo' is updated constantly, it's better to make them as an object
  ...
  _deleteToDo = (id) =>{
    //to Delete, we need to get id of todo
    this.setState(prevState =>{
  ...
  render() {
    ...
    const {newTodo, loadedToDos, toDos}= this.state; //17-2. mention loadedToDos
        ...
        <View style={styles.card}>
          ...
          //21-2 pass this._deleteToDo to <toDo />
           {Object.values(toDos).map(toDo =>(
             <toDo key={toDo.id} {...toDo} _deleteToDo={this._deleteToDo} />
            ))} 
            {/*
              18. adding ToDo list
              ...
              case 2) Object  
              in this project, we are using Object so using different method
              {Object.values(toDos)} => Array
              now we are going to map this array as we used to do
